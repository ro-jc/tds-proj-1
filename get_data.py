...
class GitHubAPI:
    ...
    def search_mumbai_users(
        self, min_followers: int = 50, max_results: int = 1000
    ) -> List[Dict]:
...
def save_data(token: str):
            ...
            users_data.append(
                {
                    ...
                    "hireable": (
                        user_details["hireable"] if user_details["hireable"] else ""
                    ),
    ...
    if users_data:
        with open("users.csv", "w") as f:
            f_csv = csv.writer(f)
            f_csv.writerow(list(users_data[0].keys()))
    ...
    if repos_data:
        with open("repositories.csv", "w") as f:
            f_csv = csv.writer(f)
            f_csv.writerow(list(repos_data[0].keys()))
            for record in repos_data:
                row = list(record.values())
    ...
    with open("raw_data.json", "w") as f:
        json.dump({"users": users_data, "repositories": repos_data}, f, indent=2)
